...
Ethernet::Ethernet(unsigned char* data, int dataSize) :
...
int Ethernet::getTotalHeaderSize() {
    return sizeof(Ethernet::s_ethernet);
}

int Ethernet::getTotalNbByteInBufferNeed() {
    return sizeof(Ethernet::s_ethernet) + sizeof(int); //Header + Frame Check Sequence (FCS)
}

std::string Ethernet::toString() {
	std::ostringstream stream;
    stream << "Ethernet(" << this->getTotalHeaderSize() << ")=[Destination : "
...
void Ethernet::setDataOnBuffer() {
    ...
    int *fcs = (int *) (this->buffer + this->bufferSize - sizeof(int));
    *fcs = this->calculFCS();
}
...
unsigned short Ethernet::getEther_type() {
...
unsigned int Ethernet::calculFCS() {
    //assert(sizeof (unsigned int) == 4);
    //massert(((unsigned int) -1) > 0);
    static const unsigned int fcstab_uint[256] =
       {
    ...
    for (int i = 0; i < this->bufferSize; i++)
        fcs = (((fcs) >> 8) ^ fcstab_uint[((fcs) ^ this->buffer[i++]) & 0xff]);
    return fcs;
}
